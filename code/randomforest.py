...
SCORING = "f1_macro"
version = "final_"
model_type = f"random_forest_{RANDOM_STATE}_{SCORING}"

print("\nSet hyperparameters")
n_components = [100]
n_estimators = [475, 500, 525, 550]
...
cv_outer = StratifiedShuffleSplit(y, test_size=0.2, random_state=RANDOM_STATE + 4)
...
df_train.to_csv(
    "../1st_gen_predictions_submission/submission{0}{1}.csv".format(version, model_type),
    mode="w",
    head=True,
    index=False,
)
...
df_train = pd.concat([probabilities_train_df, predictions_train_df], axis=1)
df_train.to_csv(
    "../1st_gen_predictions_test/test{0}{1}.csv".format(version, model_type),
    mode="w",
    head=True,
    index=False,
)

# mean_absolute_error 0 {'pca__n_components': 100, 'clf__max_depth': 7, 'clf__n_estimators': 525}
# log_loss 0 {'pca__n_components': 100, 'clf__max_depth': 6, 'clf__n_estimators': 550}
# f1 0 {'pca__n_components': 100, 'clf__max_depth': 7, 'clf__n_estimators': 525}
# roc_auc 0 {'pca__n_components': 100, 'clf__max_depth': 9, 'clf__n_estimators': 500}
